-- MPA-Index FINAL v23: one row per completed participant, suitable for CSV/Excel export.
-- This query intentionally filters to the final post-pilot questionnaire version so pilot records are not mixed into the final dataset.
...
WITH pivoted AS (
  SELECT
    ...
    MAX(answer_value) FILTER (WHERE question_id = 'OPEN1') AS participant_feedback,
    MAX(answer_value) FILTER (WHERE question_id = 'TRAIL_IMAGE_FEATURES') AS image_features_json,
    MAX(answer_value) FILTER (WHERE question_id = 'TRAIL_IMAGE_DOMINANT') AS dominant_marker_id
  FROM answers
...
image_parsed AS (
  SELECT
    p.*,
    CASE
      WHEN p.image_features_json IS NULL OR btrim(p.image_features_json) = '' THEN NULL
      ELSE p.image_features_json::jsonb
    END AS image_features
  FROM pivoted p
),
image_flat AS (
  SELECT
    p.submission_id,
    MAX(e.elem->>'tag') FILTER (WHERE e.ord = 1) AS image_marker_1_tag,
    MAX(e.elem->>'x') FILTER (WHERE e.ord = 1) AS image_marker_1_x,
    MAX(e.elem->>'y') FILTER (WHERE e.ord = 1) AS image_marker_1_y,
    MAX(e.elem->>'tag') FILTER (WHERE e.ord = 2) AS image_marker_2_tag,
    MAX(e.elem->>'x') FILTER (WHERE e.ord = 2) AS image_marker_2_x,
    MAX(e.elem->>'y') FILTER (WHERE e.ord = 2) AS image_marker_2_y,
    MAX(e.elem->>'tag') FILTER (WHERE e.ord = 3) AS image_marker_3_tag,
    MAX(e.elem->>'x') FILTER (WHERE e.ord = 3) AS image_marker_3_x,
    MAX(e.elem->>'y') FILTER (WHERE e.ord = 3) AS image_marker_3_y,
    MAX(e.elem->>'tag') FILTER (WHERE e.elem->>'id' = p.dominant_marker_id) AS dominant_image_feature_tag
  FROM image_parsed p
  LEFT JOIN LATERAL jsonb_array_elements(COALESCE(p.image_features, '[]'::jsonb)) WITH ORDINALITY AS e(elem, ord) ON TRUE
  GROUP BY p.submission_id, p.dominant_marker_id
)
SELECT
  ...
  CASE p.status
    WHEN 'madaba_resident' THEN 'Madaba resident'
    WHEN 'visitor_jordan' THEN 'Visitor from elsewhere in Jordan'
    WHEN 'international_visitor' THEN 'International visitor'
    ELSE p.status
  END AS status,
  p.age,
  CASE p.gender
    ...
    WHEN 'male' THEN 'Male'
    WHEN 'prefer_not' THEN 'Prefer not to say'
    ELSE p.gender
  END AS gender,
  CASE p.previous_use
    WHEN 'first_visit' THEN 'First visit'
    WHEN '2_5_visits' THEN '2-5 visits'
    WHEN '6_plus_visits' THEN '6+ visits'
    WHEN 'monthly' THEN 'Monthly'
    WHEN 'weekly_plus' THEN 'Weekly+'
    ELSE p.previous_use
  END AS previous_trail_use,
  CASE p.purpose_today
    WHEN 'sightseeing' THEN 'Sightseeing'
    WHEN 'shopping_services' THEN 'Shopping/services'
    WHEN 'work_study' THEN 'Work/study'
    WHEN 'social_family' THEN 'Social/family'
    WHEN 'passing_through' THEN 'Passing through'
    ELSE p.purpose_today
  END AS main_purpose_today,
  CASE p.walking_with
    WHEN 'alone' THEN 'Alone'
    WHEN 'adults_only' THEN 'Adults only'
    WHEN 'children' THEN 'Child/children'
    WHEN 'organized_tour' THEN 'Organized tour group'
    ELSE p.walking_with
  END AS walking_with,
  CASE p.time_on_trail
    WHEN '10_19' THEN '10-19 min'
    WHEN '20_39' THEN '20-39 min'
    WHEN '40_59' THEN '40-59 min'
    WHEN '60_plus' THEN '60+ min'
    ELSE p.time_on_trail
  END AS time_on_trail,
  ...
  f.image_marker_1_tag, f.image_marker_1_x, f.image_marker_1_y,
  f.image_marker_2_tag, f.image_marker_2_x, f.image_marker_2_y,
  f.image_marker_3_tag, f.image_marker_3_x, f.image_marker_3_y,
  f.dominant_image_feature_tag,
  p.participant_feedback AS open_ended_feedback,
  p.image_features_json
FROM submissions s
...
WHERE s.completion_status = 'completed'
  AND s.questionnaire_version = '23.0.0-after-pilot-3-mediators'
